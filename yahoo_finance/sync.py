...
CSV_PATH = REPO_ROOT / "out" / "yahoo_finance_prices.csv"
...
def main() -> None:
    ...
    df = validate_and_load_csv(CSV_PATH, symbol_column="yfinance_symbol")
    tickers = sorted(df["yfinance_symbol"].dropna().unique().tolist())
    print(f"CSV OK: {len(df)} rows. {len(tickers)} tickers. date range: {df['as_of_date'].min()} -> {df['as_of_date'].max()}.")

    assets = load_assets(ASSETS_PATH)
    by_ticker = { a.yfinance_symbol: a for a in assets if a.yfinance_symbol }
    validate_csv_symbols_against_assets(tickers, by_ticker, symbol_column="yfinance_symbol")
    print(f"assets.json OK.")

    fetched_prices = build_price_rows(df, by_ticker, symbol_column="yfinance_symbol")
